...
{
    public partial class GeneticAlgorithmTSPSolverParallel : ITSPSolver
    {
        private readonly ThreadLocal<Random> generator = new ThreadLocal<Random>(() => new Random(777));
        private readonly int populationSize;
        private readonly int iterations;

        public GeneticAlgorithmTSPSolverParallel(int populationSize, int iterations)
        {
            if (populationSize <= 0)
            {
        ...
        public List<int> Solve(IReadOnlyList<Location> cities)
        {
            ...
            {
                Parallel.For(0, populationSize, j =>
                {
        ...
        private Individual[] GenerateRandomPopulation(IReadOnlyList<Location> cities, out int[] tours, out Individual bestIndividual)
        {
            ...
            for (int i = 0; i < populationSize; i++)
            {
                ...
                Enumerable.Range(1, cities.Count).Shuffle(generator.Value).CopyTo(tour.AsSpan());
        ...
        {
            var individual1Idx = generator.Value.Next(population.Count);
            var winnerIdx = individual1Idx;

            int individual2Idx;
            do
            {
                individual2Idx = generator.Value.Next(population.Count);
            }
            ...
            do
            {
                individual3Idx = generator.Value.Next(population.Count);
            }
        ...
        private void OrderCrossover(ArraySegment<int> childTour, ArraySegment<int> parent1Tour, ArraySegment<int> parent2Tour)
        {
            ...
            var startIndex = generator.Value.Next(cityCount);
            var endIndex = generator.Value.Next(cityCount);
        ...
        {
            if (generator.Value.NextDouble() >= mutationProbability)
            {
                return;
            }
            int index1 = generator.Value.Next(child.Count);
